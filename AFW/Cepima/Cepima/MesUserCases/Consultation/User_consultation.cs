                ...
                using (MySqlTransaction tr = con.BeginTransaction())
                {
                    try
                    {
                        ...
                        using (MySqlCommand cmd = new MySqlCommand(queryConsultation, con, tr))
                        {
                            ...
                            cmd.Parameters.AddWithValue("@prix", tb_montant.Value);
                            cmd.ExecuteNonQuery();




                            int id_facture = MesClasses.ReceptionManager.CreerFactureSiInexistante(ID_PATIENT.ToString(), con, tr);
                            MesClasses.ReceptionManager.MettreAJourPrestation(id_facture, "Consultation", 1, tb_montant.Value, tb_montant.Value, con, tr);
                        }
                        tr.Commit();
