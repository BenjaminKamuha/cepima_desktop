        ...
        private void FormFacturePrint_Load(object sender, EventArgs e)
        {
            using (MySqlConnection con = MesClasses.ManagerClasse.GetConnexion())
            {
                try
                {
                    ...
                    string queryFacture = "SELECT f.id_facture,f.date_facture,f.type_facture,f.montant_total,f.statut,CONCAT(p.nom,' ',p.post_nom,' ',p.prenom) AS patient FROM facture f JOIN patients p ON f.id_patient = p.id_patient WHERE f.id_facture = @id";
